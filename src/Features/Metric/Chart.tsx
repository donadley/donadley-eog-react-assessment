...
const useSubscribe = () => {
    ...
    const newMeasurements: Map<string, Measurement[] | null> = useSelector(selectors.getNewMeasurements);
...
const Chart = () => {
  ...
  useEffect(() => {
    ...
    getMultipleMeasurements.forEach( (measurements: { metric: string; measurements: Measurement[]; })  => {
        newMeasurements.set(measurements.metric, measurements && measurements.measurements[0] != undefined ? measurements.measurements : null);
      })
  ...
  return <LineChart metrics={newMeasurements} />;
};
